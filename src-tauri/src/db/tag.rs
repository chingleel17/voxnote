...
pub async fn create_tag(pool: &SqlitePool, name: &str, color: &str) -> Result<Tag, sqlx::Error> {
    ...
    .execute(pool)
    .await?;
    Ok(Tag { id, name: name.to_string(), color: color.to_string(), created_at: now })
}
...
pub async fn set_meeting_tags(pool: &SqlitePool, meeting_id: &str, tag_ids: &[String]) -> Result<(), sqlx::Error> {
...
pub async fn get_meeting_tags(pool: &SqlitePool, meeting_id: &str) -> Result<Vec<Tag>, sqlx::Error> {
    let rows = sqlx::query_as::<_, Tag>(

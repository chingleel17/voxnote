...
即時字幕目前的「電腦音訊」來源是**整台裝置的輸出**（`start_windows_loopback_capture` 以 `Direction::Render` 取預設輸出裝置後開 loopback）。使用者參與線上會議、觀看直播，或處理無法預先取得完整媒體的其他即時內容時，此擷取方式會一併收錄同時段的所有系統聲音——通知音效、其他應用程式的自動播放、音樂播放器與會議軟體提示音。這些非目標音訊會與目標語音混在同一個分析視窗送入模型，降低轉錄品質。

Windows build 20348 起提供 **process loopback**（`ActivateAudioInterfaceAsync` 搭配 `AUDIOCLIENT_ACTIVATION_PARAMS`），可指定行程 ID 只擷取該行程及其子行程的音訊輸出。專案既有的 `wasapi` 0.23 相依**已封裝此能力**（`AudioClient::new_application_loopback_client(process_id, include_tree)`，已於 crate 原始碼與 Microsoft 官方範例確認），無須新增外部相依。
...
- **當次 session 選取目標**：行程 ID 僅作為當次啟動參數，不寫入持久設定；應用程式重啟後須由使用者從最新清單重新選取，避免沿用失效或已被重用的 PID。
- **目標應用程式結束的處理**：被擷取的應用程式於 session 進行中結束時，回報原因並結束 session，而非靜默停止產生字幕。

本變更不修改既有的麥克風與全系統音訊擷取行為，不影響批次錄音流程，無 breaking change。

對已存在且可存取的非直播 YouTube 影片，`add-video-url-subtitle` 所定義的「先完成整支轉錄，再以內嵌播放器時間同步」是較可靠的主要路徑。本能力不取代該流程；它服務無法預先取得完整媒體或確實需要邊播放邊辨識的即時情境。
...
本變更與 `add-live-caption-remote-asr` 無相依，兩者可獨立進行。它也不改變 `add-streaming-incremental-caption` 的解碼與 LocalAgreement 政策；指定應用程式只改變音訊擷取來源，增量模式啟用或關閉時都必須沿用同一條後續字幕處理路徑。
...
- `src/types/index.ts`：新增應用程式選項與 session 啟動請求型別；`AppConfig` 僅擴充音訊來源枚舉，不保存 PID。
...
- `get_mixformat()` 回傳 `Not implemented`——process loopback 不得呼叫；初始化成功後直接以要求的 `desired_format` 為實際交付格式。
- `get_device_period()` 回傳 `Not implemented`——process loopback 須改為傳入自訂 buffer duration（crate 文件指出該值於此模式下不影響行為）。
- `initialize_client` 必須使用 `Direction::Capture` 與共用模式。

**平台限制**
- 僅 Windows build 20348 以上支援。低於此 build 或非 Windows 平台，此音訊來源不可用，須在呈現選項與啟動前回報。

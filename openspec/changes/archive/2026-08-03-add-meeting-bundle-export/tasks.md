...
- [x] 5.0 新增 8 項音訊檔名單元測試（`cargo test --lib meeting_export`），涵蓋副檔名分歧、桌面錄音無原始檔名、非法字元與路徑跳脫；過程中發現並修正 `Path::file_stem()` 會把 `/` 當路徑分隔符而截掉前半段檔名的缺陷

- [x] 5.1 執行 `npm run build` 確認 TypeScript 編譯通過
- [x] 5.2 執行 `npm run tauri dev`，實測：多段錄音會議匯出後子資料夾含全部音訊、序號順序正確
- [x] 5.3 實測逐字稿版本一致性：切換至原始版/校稿版/手動版分別匯出，確認輸出版本與畫面一致且 `meeting-info.md` 標示正確
- [x] 5.4 實測摘要不存在時匯出仍成功、且子資料夾不含摘要檔
- [x] 5.5 實測 `meeting_date` 為空的會議，子資料夾日期退回 `created_at`
- [x] 5.6 實測含非法檔名字元的會議標題可正常建立資料夾：`Q3/Q4 檢討：進度?` 應產生 `Q3_Q4 檢討：進度_`（全形 `：` 保留）
- [x] 5.7 實測有錄音但完全無逐字稿的會議：匯出成功、子資料夾不含逐字稿檔、`meeting-info.md` 標示逐字稿為「無」
- [x] 5.8 實測重複匯出同一會議時出現覆寫確認；分別驗證確認覆寫與取消兩種結果
- [x] 5.9 實測錄音檔遺失情境（手動移走 `file_path` 指向的檔案）：匯出仍完成，warning toast 列出被跳過項目
- [x] 5.10 確認既有逐字稿單檔匯出與摘要單檔匯出行為未受影響
